...
def generate_fibonacci_sphere_points(samples=256):
    points = []
    phi = (1 + np.sqrt(5)) / 2
...
def find_closest_point_index(points, target_vector):
    target_vector = target_vector / np.linalg.norm(target_vector)
    dot_products = np.dot(points, target_vector)
    return np.argmax(dot_products)

def get_position_on_meridian(progress):
    latitude = np.pi * (progress - 0.5)
    x = np.cos(latitude) * -1
    y = 0.0
    z = np.sin(latitude)
    return np.array([x, y, z])

fib_points = generate_fibonacci_sphere_points(256)
...
def simulator( 
        ...
        stimulus_location = np.array([0, 0, 0, 0])*np.pi, # from 0 to np.pi
    ...
    PENx = NeuronGroup(48,model=eqs_PENx, threshold='v>Vth', reset='v=Vr', refractory='1*ms', method='euler')
    ...
    PENx_groups = [PENx[i:i+3] for i in range(0, 48, 3)]
    R_groups = [R[0:3]]
    ...
    if debug: print(f'{time.strftime("%H:%M:%S")} [info] Connections done')
    # ========= end PEN -> EPG =========
    ...
    PRM_PEN0 = PopulationRateMonitor(PENx_groups[0])
    PRM_PEN1 = PopulationRateMonitor(PENx_groups[1])
    PRM_PEN2 = PopulationRateMonitor(PENx_groups[2])
    PRM_PEN3 = PopulationRateMonitor(PENx_groups[3])
    PRM_PEN4 = PopulationRateMonitor(PENx_groups[4])
    PRM_PEN5 = PopulationRateMonitor(PENx_groups[5])
    PRM_PEN6 = PopulationRateMonitor(PENx_groups[6])
    PRM_PEN7 = PopulationRateMonitor(PENx_groups[7])
    PRM_PEN8 = PopulationRateMonitor(PENx_groups[8])
    PRM_PEN9 = PopulationRateMonitor(PENx_groups[9])
    PRM_PEN10 = PopulationRateMonitor(PENx_groups[10])
    PRM_PEN11 = PopulationRateMonitor(PENx_groups[11])
    PRM_PEN12 = PopulationRateMonitor(PENx_groups[12])
    PRM_PEN13 = PopulationRateMonitor(PENx_groups[13])
    PRM_PEN14 = PopulationRateMonitor(PENx_groups[14])
    PRM_PEN15 = PopulationRateMonitor(PENx_groups[15])
    
    PRM_R0 = PopulationRateMonitor(R_groups[0])

    # mon_R = StateMonitor(R, ['v', 'IsynEI', 'Isyn_ii'], record=True)
    # mon_PEN = StateMonitor(PEN, ['v', 'Isyn_pp', 'Isyn_EP'], record=True)
    # mon_EPG = StateMonitor(EPG_0, ['v', 'Isyn', 'Isyn_i', 'Isyn_PE'], record=True)
    # mon_EPG_1 = StateMonitor(EPG_1, ['v', 'Isyn', 'Isyn_i', 'Isyn_PE'], record=True)
    # mon_EPG_2 = StateMonitor(EPG_2, ['v', 'Isyn', 'Isyn_i', 'Isyn_PE'], record=True)
    # mon_EPG_3 = StateMonitor(EPG_3, ['v', 'Isyn', 'Isyn_i', 'Isyn_PE'], record=True)
    ...
    Dy = 1
    Dt = 100 # ms
    A = stimulus_strength[0]

    # for k in trange(30):
    #     stimulus_location += Dx
    ...
    #     for i in range(0,8):
    #         EPG_groups[int((i+k*Dy*16)%256)].I = visual_cue(theta_r[0], i, 0.3)
    #         EPG_groups[int((i+8+k*Dy*16)%256)].I = visual_cue(theta_l[0], i+8, 0.3)
    #     net.run(Dt * ms)
    ...
    theta_l = theta_r + np.pi
    for i in range(8):
        EPG_groups[i].I = visual_cue(theta_r[0], i, 0.3)
        EPG_groups[i+8].I = visual_cue(theta_l[0], i+8, 0.3)
    ...
    # if debug:
    #     # Use tqdm for progress tracking during no stimulus phase
    #     no_stimulus_steps = 10
    #     no_stimulus_step_duration = t_epg_close / no_stimulus_steps
        
    #     with tqdm(total=no_stimulus_steps, desc="Visual cue off", unit="step") as pbar:
    #         for step in range(no_stimulus_steps):
    #             net.run(no_stimulus_step_duration * ms)
    #             pbar.update(1)
    # else: net.run(t_epg_close * ms)


    # if half_PEN == 'right': 
    #     for i in range(8): 
    #         PENx_groups[i].I = shifter_strength
    # elif half_PEN == 'left': 
    #     for i in range(8,16): 
    #         PENx_groups[i].I = shifter_strength
    # else: raise ValueError("half_PEN must be 'right' or 'left'")

    # if debug:
    #     # Use tqdm for progress tracking during shifting simulation
    #     shifting_steps = 10
    #     shifting_step_duration = t_pen_open / shifting_steps
        
    #     with tqdm(total=shifting_steps, desc="PEN on", unit="step") as pbar:
    #         for step in range(shifting_steps):
    #             net.run(shifting_step_duration * ms)
    #             pbar.update(1)
    # else:
